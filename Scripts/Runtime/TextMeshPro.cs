    ...
    public partial class TextMeshPro : TMP_Text, ILayoutElement
    {
        ...
        public override void UpdateVertexData(TMP_VertexDataUpdateFlags flags)
        {
            ...
            for (int i = 0; i < materialCount; i++)
            {
                ...
                if ((flags & TMP_VertexDataUpdateFlags.Uv3) == TMP_VertexDataUpdateFlags.Uv3)
                    mesh.uv3 = m_textInfo.meshInfo[i].uvs3;

                if ((flags & TMP_VertexDataUpdateFlags.Colors32) == TMP_VertexDataUpdateFlags.Colors32)
                    mesh.colors32 = m_textInfo.meshInfo[i].colors32;

                if ((flags & TMP_VertexDataUpdateFlags.Normals) == TMP_VertexDataUpdateFlags.Normals) // outline param
                    mesh.normals = m_textInfo.meshInfo[i].normals;

                if ((flags & TMP_VertexDataUpdateFlags.Tangents) == TMP_VertexDataUpdateFlags.Tangents) // outline color
